...
class LLMEvaluator {
    ...
    func load(modelName: String) async throws -> ModelContainer {
        ...
        case .idle:
            ...
            modelInfo =
            "Loaded \(modelConfiguration.name).  Weights: \(MLX.GPU.activeMemory / 1024 / 1024)M"
                ...
                let input = try await context.processor
                    .prepare(
                        input: .init(
                            messages: [
                                ["role": "system",
                                 "content": modelConfiguration.formatForTokenizer(systemPrompt),],
                                ["role": "user",
                                "content": modelConfiguration.formatForTokenizer(prompt),],
                            ]
                        )
                    )
